...
export default function HighlightModal({ onClose }: { onClose: () => void }) {
  ...
  useState(() => {
    ...
    const charNames = getCharacters().map(c => c.name)

    analyzeHighlights(dialogues, title, charNames).then(result => {
      if (result.length > 0) {
  ...
  return (
    <div className="gx-overlay" onClick={onClose}>
      ...
      >
        ...
        {error && (
          <div style={{ padding: 10, borderRadius: 8, background: 'rgba(220,38,38,0.1)', color: '#ef4444', fontSize: 12, marginBottom: 12 }}>
            {error}
          </div>
        )}
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <button
                ...
                style={{
                  ...
                  background: outputType === 'comic' ? 'var(--primary)' : 'var(--bg-card)',
                  color: outputType === 'comic' ? '#fff' : 'var(--text-secondary)',
              ...
              <button
                ...
                style={{
                  flex: 1, padding: 8, borderRadius: 8, border: '1px solid var(--border)', cursor: 'pointer',
                  background: outputType === 'video' ? 'var(--primary)' : 'var(--bg-card)',
                  color: outputType === 'video' ? '#fff' : 'var(--text-secondary)',
                  fontWeight: 600, fontSize: 13,
                }}
              >
                🎬 视频
