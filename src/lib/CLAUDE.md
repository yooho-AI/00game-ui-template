...
```
store.ts      : Zustand+Immer 游戏状态中枢，cfg() 动态配置解析，_worldConfig 注入，存档 v2，getCharacterColors()
generator.ts  : AI 世界生成器 — chat() 调用 Gemini，extractJson/validate/normalize，generateWorld() 含自动重试
